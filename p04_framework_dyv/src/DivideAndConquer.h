...
class DivideAndConquer {
 public:
   ...
   * @return Solution<S> 
   */
  Solution<S> Solve(const Problem<P>& problem, int length, int how_many = 2) {
    try {
      if (isSmall(problem, length)) {
        ...
        for (int i = 0; i < how_many; i++) {
          if (solutions[i].get_level() > actual_max_level) {
            actual_max_level = solutions[i].get_level();
          }
        }
        Solution<S> solution = combine(solutions);
        solution.set_level(actual_max_level + 1);
        return solution;
      }
  ...
  virtual bool isSmall(const Problem<P>&, int length) = 0;
  virtual std::string get_recursivity() = 0;
  virtual std::string get_factor() = 0;
  virtual std::string get_combine_complex() = 0;
  virtual std::vector<Problem<P>> divide(const Problem<P>&, int length, int how_many = 2) = 0;
