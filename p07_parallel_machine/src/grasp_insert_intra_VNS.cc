...
Solution GraspInsertIntraVNS::localSearch(Solution solution) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = 0; j < tasks.size(); j++) {
        for (int k = 0; k < tasks.size(); k++) {
          ...
          int new_tct = new_problem.calculate_tct(posible_new_solution);
          if (new_tct < best_tct) {
...
int GraspInsertIntraVNS::tct_intra(int base_tct, const std::vector<int>& tasks, int position, int new_position, int element, Problem& problem) {
