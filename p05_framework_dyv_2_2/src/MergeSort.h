 ...
 */
   ...
   * @param length 
   * @param how_many 
   * @return std::vector<BaseProblem<P>> 
   */
  std::vector<BaseProblem<P>*> divide(const BaseProblem<P>* problem, int length, int how_many) override {
    ...
    for (int i = 0; i < how_many; i++) {
      BaseProblem<P>* subproblem = new Problem<P>();
  ...
  Solution<S>* combine(const std::vector<Solution<S>*>& solutions,const BaseProblem<P>* problem) override {
    ...
    bool done = false;
    std::vector<int> indexes(solutions.size(), 0);
    int auxiliar = 10000;
    int actual_index = -1;
    while (!done) {
      done = true;
      ...
      if (!done) {
        combined->push_back(auxiliar); // Accede al objeto mediante el operador de flecha
        auxiliar = 10000;
        actual_index = -1;
      }
    }
